...
exports.run = async (client, message, args) => {
  ...
  if (message.author.id !== '392362046714281984') return message.channel.send(çarpı + " Bu komutu sadece `Yapımcım` **kullanabilir!**")
  let nesne = args[0]
  let çarpı2 = client.emojis.get(":boost:")
  if (!nesne) return message.channel.send(çarpı2 + ' **Gold üye olacak kişinin idsini girermisin başkan.**')
  
 await db.set(`gold_${nesne}`, 'tm')
  let tik = client.emojis.get("392362046714281984")
  message.channel.send(`${tik} <@${nesne}> kullanıcısı yani **${nesne}** IDli kullanıcı gold üye oldu!`)
   client.guilds.get("392362046714281984").channels.find(x => x.name === "➵gold-üye-log").send(`${tik} <@${nesne}> kullanıcısı için gold üyeliği **açıldı!**`)
